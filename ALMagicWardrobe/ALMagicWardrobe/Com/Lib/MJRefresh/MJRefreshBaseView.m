...
{
    __weak ALLabel *_statusLabel;
    __weak ALImageView *_arrowImage;
...
{
    if (!_statusLabel) {
        ALLabel *statusLabel = [[ALLabel alloc] initWithFrame:CGRectZero BoldFont:13 BGColor:RGB_BG_Clear FontColor:MJRefreshLabelTextColor];
        statusLabel.autoresizingMask = UIViewAutoresizingFlexibleWidth;
//        statusLabel.textAlignment = NSTextAlignmentCenter;
        statusLabel.verticalAlignment = VerticalAlignmentBottom;
 ...
 *  箭头图片
 */
- (ALImageView *)arrowImage
{
    if (!_arrowImage) {
        ALImageView *arrowImage = [[ALImageView alloc] initWithImage:[UIImage imageNamed:MJRefreshSrcName(@"arrow.png")]];
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.backgroundColor = RGB_BG_Clear;
