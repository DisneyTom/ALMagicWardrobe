...
- (void)setMode:(OcnProgressHUDMode)newMode {
...
+ (void)showHUDViewTo:(UIView *)view
    ...
    ALProgressHUD *hud = [[ALProgressHUD alloc] initWithView:view];
    hud.removeFromSuperViewOnHide = YES;
    hud.opacity = 0.5;
    hud.customView = [[ALImageView alloc] initWithImage:image];
    hud.labelText = text;
...
+ (ALProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    [ALProgressHUD hideHUDForView:view animated:animated];
	ALProgressHUD *hud = [[ALProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[ALProgressHUD class]]) {
            ALProgressHUD *HUD = (ALProgressHUD *)v;
            HUD.removeFromSuperViewOnHide = YES;
            [HUD hide:animated];
		}
	}
...
//		[self.labelText sizeWithFont:self.labelFont constrainedToSize:CGSizeMake(200, 50) lineBreakMode:NSLineBreakByCharWrapping];
        CGSize dims = [ALComAction getSizeByStr:self.labelText andFont:self.labelFont andRect:CGSizeMake(200, 50)];
