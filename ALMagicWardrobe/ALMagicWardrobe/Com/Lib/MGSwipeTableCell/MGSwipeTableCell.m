...
-(instancetype) initWithButtons:(NSArray*) buttonsArray direction:(MGSwipeDirection) direction
{
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, maxSize.width * buttonsArray.count, maxSize.height)]) {
        ...
        container.backgroundColor = RGB_BG_Clear;
...
-(void) createSwipeViewIfNeeded
{
    if (!swipeOverlay) {
        ...
        swipeView = [[ALImageView alloc] initWithImage:[self imageFromView:self]];
...
-(UIColor *) backgroundColorForSwipe
{
    ...
    else if (self.contentView.backgroundColor && ![self.contentView.backgroundColor isEqual:RGB_BG_Clear]) {
        return self.contentView.backgroundColor;
    }
    else if (self.backgroundColor && ![self.backgroundColor isEqual:RGB_BG_Clear]) {
